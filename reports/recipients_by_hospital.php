...
$query = "SELECT r.id AS recipient_id, r.name AS recipient_name, h.id AS hospital_id, h.name AS hospital_name
          FROM recipient r
          JOIN hospital h ON r.hospital_id1 = h.id
          ORDER BY h.id, r.id";
...
head>
    ...
    <title>Recipients by Hospital</title>
</head>
<body>
...
<div class="container">
    <h2>Recipients by Hospital</h2>
    <p>Here are the recipients along with their associated hospital details.</p>

    <?php
    if ($stmt->rowCount() > 0) {
        echo "<table><tr><th>Recipient ID</th><th>Recipient Name</th><th>Hospital ID</th><th>Hospital Name</th></tr>";
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr><td>".$row["recipient_id"]."</td><td>".$row["recipient_name"]."</td><td>".$row["hospital_id"]."</td><td>".$row["hospital_name"]."</td></tr>";
        }
        echo "</table>";
    } else {
