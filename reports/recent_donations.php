...
$query = "SELECT d.name AS donor_name, de.date, bb.location AS blood_bank_location
          FROM donor d
          JOIN donation_event de ON d.id = de.donor_id
          JOIN blood_bank bb ON de.bloodBank_id = bb.id
          WHERE de.date >= CURDATE() - INTERVAL 1 MONTH
...
<div class="container">
    ...
    <p>Here are the most recent donations within the last month.</p>

    <?php
    if ($stmt->rowCount() > 0) {
        echo "<table><tr><th>Donor Name</th><th>Date</th><th>Blood Bank Location</th></tr>";
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr><td>".$row["donor_name"]."</td><td>".$row["date"]."</td><td>".$row["blood_bank_location"]."</td></tr>";
        }
        echo "</table>";
    } else {
